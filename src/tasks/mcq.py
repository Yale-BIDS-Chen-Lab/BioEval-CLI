# Multiple Choice Questions (MCQ) task implementation

from typing import Dict, List

class MCQTask:
    """Handler for Multiple Choice Questions tasks"""
    
    def postprocess(self, output: str, label_string: str = "A-E") -> str:
        """
        Process model output for MCQ tasks.
        
        Note: This method should not be used. Use postprocessing functions 
        specified in the YAML config instead (e.g., process_mcq_custom).
        
        Args:
            output (str): The model output string
            label_string (str): Not used
            
        Returns:
            str: The raw output (no processing)
        """
        return output if output else ""
    
    def get_metrics(self) -> List[str]:
        """Get list of metrics for MCQ tasks"""
        return ["accuracy", "macro_f1", "weighted_f1"]
    
    def add_example_metadata(self, record: Dict) -> Dict:
        """Add example-level metadata for MCQ tasks"""
        # MCQ tasks currently don't need additional example-level metadata
        # The postprocessed_output and reference are sufficient
        return record 